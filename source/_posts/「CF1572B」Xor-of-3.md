...
当 $n$ 为奇数时，有一个构造：
- 先选择 $n - 2, n - 4, \dots, 3, 1$ 进行操作。此时 $a_1 = 0$（即所有数的异或和），且 $a_{2i} = a_{2i + 1}$。
- 再选择 $1, 3, \dots, n - 4, n - 2$ 进行操作。此时每次操作的位置 $x$ 必定满足 $a_x = 0, a_{x + 1} = a_{x + 2}$，故该次操作会使得 $a_x = a_{x + 1} = a_{x + 2} = 0$。
