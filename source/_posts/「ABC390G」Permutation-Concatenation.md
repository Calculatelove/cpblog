...
对于一个整数序列 $a_1, a_2, \cdots, a_n$，考虑 $a_i$ 对 $f(A)$ 的贡献，实际上即为 $a_i \times 10^{L}$（其中 $L$ 表示 $\overline{a_{i + 1}\cdots a_n}$ 的位数）。

现在考虑计算一个数 $v$ 对整体答案的贡献。枚举其在排列中的位置 $i$，则此时要考虑 $i + 1 \sim n$ 部分的影响。等价于在 $\{1, \cdots, n \} \setminus \{v\}$ 中选出 $n - i$ 个数，每当选到一个位数为 $d$ 的数，就会使得贡献乘上 $10^d$。求所有选法的贡献之和。
