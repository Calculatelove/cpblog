...
在游戏开始前，Alice 可以决定房间的访问顺序，访问顺序可以用一个排列 $p$ 描述，表示依次经过房间 $p_1, p_2, \dots, p_n$，只有房间 $p_i$ 没有石子的情况下，才可以去房间 $p_{i + 1}$。
...
<span class="highlight">反 NIM 博弈</span>：对于若干堆石子 $a_1, a_2, \dots, a_m$，**最后一个将石子取完的玩家失败**。
