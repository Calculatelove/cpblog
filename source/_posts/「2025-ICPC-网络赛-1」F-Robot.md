...
将边界填充完整之后，考虑对框住的矩形内部进行查杀。我们大可不必 $99 \times 99$ 地毯式查杀。考虑每次对于框住的矩形区域，取出最长的一条边，填充这条边的中线。然后对剩下的矩形继续进行操作，直到只剩下 $1\times 1$ 的矩形为止。设边长为 $L$，则所需要的步数为 $L + \frac{L}{2} + \frac{L}{2} + \frac{L}{4} + \frac{L}{4} + \cdots = \mathcal{O}(L)$。
